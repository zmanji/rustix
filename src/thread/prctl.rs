...
pub fn set_keep_capabilities(enable: bool) -> io::Result<()> {
    unsafe { prctl_2args(PR_SET_KEEPCAPS, ptr::invalid_mut(usize::from(enable))) }.map(|_r| ())
}
...
pub fn set_secure_computing_mode(mode: SecureComputingMode) -> io::Result<()> {
    unsafe { prctl_2args(PR_SET_SECCOMP, ptr::invalid_mut(mode as usize)) }.map(|_r| ())
}
...
pub fn capability_is_in_bounding_set(capability: Capability) -> io::Result<bool> {
    unsafe { prctl_2args(PR_CAPBSET_READ, ptr::invalid_mut(capability as usize)) }.map(|r| r != 0)
}
...
pub fn remove_capability_from_bounding_set(capability: Capability) -> io::Result<()> {
    unsafe { prctl_2args(PR_CAPBSET_DROP, ptr::invalid_mut(capability as usize)) }.map(|_r| ())
}
...
pub fn set_capabilities_secure_bits(bits: CapabilitiesSecureBits) -> io::Result<()> {
    unsafe { prctl_2args(PR_SET_SECUREBITS, ptr::invalid_mut(bits.bits() as usize)) }.map(|_r| ())
}
...
pub fn set_current_timer_slack(value: Option<NonZeroU64>) -> io::Result<()> {
    ...
    unsafe { prctl_2args(PR_SET_TIMERSLACK, ptr::invalid_mut(value)) }.map(|_r| ())
}
...
pub fn set_no_new_privs(no_new_privs: bool) -> io::Result<()> {
    unsafe {
        prctl_2args(
            PR_SET_NO_NEW_PRIVS,
            ptr::invalid_mut(usize::from(no_new_privs)),
        )
    }
    .map(|_r| ())
}
...
pub fn disable_transparent_huge_pages(thp_disable: bool) -> io::Result<()> {
    unsafe {
        prctl_2args(
            PR_SET_THP_DISABLE,
            ptr::invalid_mut(usize::from(thp_disable)),
        )
    }
    .map(|_r| ())
}
...
pub fn capability_is_in_ambient_set(capability: Capability) -> io::Result<bool> {
    let cap = ptr::invalid_mut(capability as usize);
...
pub fn clear_ambient_capability_set() -> io::Result<()> {
    unsafe { prctl_2args(PR_CAP_AMBIENT, ptr::invalid_mut(PR_CAP_AMBIENT_CLEAR_ALL)) }.map(|_r| ())
}
...
pub fn configure_capability_in_ambient_set(capability: Capability, enable: bool) -> io::Result<()> {
    ...
    unsafe { prctl_3args(PR_CAP_AMBIENT, ptr::invalid_mut(sub_operation), cap) }.map(|_r| ())
}
...
pub unsafe fn set_sve_vector_length_configuration(
    ...
    prctl_2args(PR_SVE_SET_VL, ptr::invalid_mut(bits as usize)).map(|_r| ())
}
...
pub unsafe fn reset_pointer_authentication_keys(
    ...
    prctl_2args(PR_PAC_RESET_KEYS, ptr::invalid_mut(keys as usize)).map(|_r| ())
}
...
pub unsafe fn set_current_tagged_address_mode(
    ...
    prctl_2args(PR_SET_TAGGED_ADDR_CTRL, ptr::invalid_mut(config as usize)).map(|_r| ())
}
...
pub unsafe fn disable_syscall_user_dispatch() -> io::Result<()> {
    prctl_2args(
        PR_SET_SYSCALL_USER_DISPATCH,
        ptr::invalid_mut(PR_SYS_DISPATCH_OFF),
    )
    .map(|_r| ())
}
...
pub unsafe fn enable_syscall_user_dispatch(
    ...
    syscalls::prctl(
        PR_SET_SYSCALL_USER_DISPATCH,
        ptr::invalid_mut(PR_SYS_DISPATCH_ON),
        always_allowed_region.as_ptr() as *mut _,
        ptr::invalid_mut(always_allowed_region.len()),
...
pub fn core_scheduling_cookie(pid: Pid, scope: CoreSchedulingScope) -> io::Result<u64> {
    ...
    unsafe {
        syscalls::prctl(
            PR_SCHED_CORE,
            ptr::invalid_mut(PR_SCHED_CORE_GET),
            ptr::invalid_mut(pid.as_raw_nonzero().get() as usize),
            ptr::invalid_mut(scope as usize),
...
pub fn create_core_scheduling_cookie(pid: Pid, scope: CoreSchedulingScope) -> io::Result<()> {
    unsafe {
        syscalls::prctl(
            PR_SCHED_CORE,
            ptr::invalid_mut(PR_SCHED_CORE_CREATE),
            ptr::invalid_mut(pid.as_raw_nonzero().get() as usize),
            ptr::invalid_mut(scope as usize),
            ptr::null_mut(),
        )
...
pub fn push_core_scheduling_cookie(pid: Pid, scope: CoreSchedulingScope) -> io::Result<()> {
    unsafe {
        syscalls::prctl(
            PR_SCHED_CORE,
            ptr::invalid_mut(PR_SCHED_CORE_SHARE_TO),
            ptr::invalid_mut(pid.as_raw_nonzero().get() as usize),
            ptr::invalid_mut(scope as usize),
            ptr::null_mut(),
        )
...
pub fn pull_core_scheduling_cookie(pid: Pid, scope: CoreSchedulingScope) -> io::Result<()> {
    unsafe {
        syscalls::prctl(
            PR_SCHED_CORE,
            ptr::invalid_mut(PR_SCHED_CORE_SHARE_FROM),
            ptr::invalid_mut(pid.as_raw_nonzero().get() as usize),
            ptr::invalid_mut(scope as usize),
            ptr::null_mut(),
        )
